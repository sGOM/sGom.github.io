...
원인은 `@Transactional`의 전파 속성과 rollback-only 표시에 있다.

## 동작 원리

REQUIRED로 참여한 두 메서드가 물리 트랜잭션 하나를 공유할 때, 예외와 표시가 어디로 흐르는지가 이렇다. 번호는 아래 「직접 확인」의 코드와 같다.

```text
outer()  @Transactional(REQUIRED)      물리 트랜잭션이 여기서 열린다
  │
  │  ① save(OUTER_MESSAGE)
  │
  ├─ inner()  @Transactional(REQUIRED)  참여만 한다. 새 트랜잭션이 아니다
  │    │  ③ save(INNER_MESSAGE)
  │    └─ throw RuntimeException
  │          프록시를 빠져나가는 순간 물리 트랜잭션에 rollback-only 표시
  │
  │  catch (e) { }                      예외는 여기서 멈춘다. 표시는 남는다
  │  ② save(RECOVERY_MESSAGE)
  │
  └─ 커밋 시도                          표시를 보고 ①②③ 전부 롤백
                                        UnexpectedRollbackException
```

REQUIRED로 참여한 안쪽 메서드에서 예외가 발생하면, 그 예외가 프록시를 빠져나가는 순간 공유 중인 물리 트랜잭션에 rollback-only 표시가 붙는다. 바깥이 이 예외를 try-catch로 잡아 정상 종료해도 표시는 남는다. 커밋 시점에 `UnexpectedRollbackException`이 나며 전체가 롤백된다.

"예외를 분명히 잡았는데 왜 롤백되는가"의 답이 여기 있다. REQUIRED로 참여한 메서드들은 논리적으로는 여러 트랜잭션이지만 실제로는 하나의 물리 트랜잭션을 공유하므로, 일부만 롤백하는 것은 불가능하다.
...
검증한 시나리오는 하나다. 바깥이 저장하고, 안쪽을 부르고, 안쪽이 저장한 뒤 예외를 던지고, 바깥이 그 예외를 잡은 다음 복구용 저장을 한 번 더 한다.

여기서 안쪽의 전파 속성만 바꿔가며 결과를 비교한다. Spring Boot + Kotlin + Kotest 환경에서 확인했다.
...
REQUIRES_NEW는 본 작업이 실패해도 남겨야 하는 기록(로그, 이력, 알림 발송 기록)을 저장할 때 쓴다. 다만 별도 커넥션을 쓰므로 커넥션 풀 크기를 함께 봐야 하고, 부모가 잠근 행을 자식이 다시 잠그면 자기 자신과 데드락(self-deadlock)에 빠진다.

NESTED는 세이브포인트 기반이라 자식만 부분 롤백할 수 있다. 바깥 트랜잭션이 롤백되면 세이브포인트 이전 상태를 포함해 전체가 함께 롤백된다.
