...
**자기 자신 행(depth = 0)을 빼먹기 쉽다.** 조상-자손 관계만 생각하면 자기 참조 행이 필요 없어 보이지만, 이 행이 없으면 "노드 3을 포함한 자신과 자손 전체"처럼 자기 자신을 결과에 포함해야 하는 조회에서 `depth >= 0` 조건을 걸어도 자기 자신이 결과에 들어오지 않는다.
...
- Bill Karwin, *SQL Antipatterns* — Closure Table을 이 이름으로 널리 알린 책. 패턴 자체는 그 전에 Vadim Tropashko, *SQL Design Patterns*(2006)가 먼저 다뤘다
