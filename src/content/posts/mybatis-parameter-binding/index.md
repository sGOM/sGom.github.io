...
- **바인딩(binding)**: `?` 자리에 값을 전달하는 것. 값은 SQL 문장과 분리된 경로로 드라이버에 넘어간다. 아래 그림의 ②③④가 이 경로다.
- **문자열 치환(string substitution)**: SQL 문장을 만들기 전에 그 자리에 문자열을 그대로 이어 붙이는 것. 붙은 문자열은 SQL 문장의 일부가 된다. 아래 그림의 ①에서 끝난다.
- **[TypeHandler](https://mybatis.org/mybatis-3/configuration.html#typeHandlers)**: 자바 타입과 JDBC 타입 사이를 변환하는 MyBatis 구성 요소. `#{}` 값은 이 경로를 거쳐 `PreparedStatement`에 설정된다. 아래 그림의 ③이다.

## 핵심 정리

매퍼 XML 한 줄이 DB에 도착하기까지 거치는 단계다. `${}`는 ①에서 끝나고, `#{}`는 ②부터 ④까지 이어진다.

```text
매퍼 XML   SELECT * FROM users WHERE name = #{name} ORDER BY ${orderBy}
  │
  │  ① MyBatis · 문자열 치환
  │     ${orderBy} 자리에 문자열 created_at을 그대로 이어 붙인다
  │
  │  ② MyBatis · 파라미터 파싱
  │     #{name}을 ?로 바꾸고, 넘길 값을 따로 모은다
  │     여기서 SQL 문장이 확정된다
  │     SELECT * FROM users WHERE name = ? ORDER BY created_at
  │
  │  ③ TypeHandler
  │     자바 값 "홍길동"을 JDBC 타입으로 옮겨 PreparedStatement 파라미터로 설정한다
  │
  │  ④ JDBC 드라이버
  │     문장과 값을 따로 보내거나, 값을 이스케이프해 문장에 채워 보낸다
  │
  ▼  ⑤ DB
     받은 문장을 파싱하고 실행계획을 세워 실행한다
```
...
| 문장 재사용 | ②에서 문장이 고정된다. ⑤까지 이어질지는 ④가 정한다 | 불가. 값마다 ①의 결과가 달라진다 |
...
**`#{}`가 안전한 것은 값이 문장 구조에 손댈 수 없기 때문이다.** ②에서 확정된 문장이 담은 것은 `?`까지고, 값은 ③④라는 다른 경로로 간다. ④가 값을 문장에 채워 넣는 경우에도 드라이버가 값을 리터럴 안에 가둬 두므로, `' OR 1=1 #` 같은 입력은 그런 내용의 문자열로만 남는다. 드라이버가 이걸 어떻게 해내는지는 아래 「더 깊이」에서 확인한다.

**`${}`는 그 보호 밖에 있다.** ①에서 이어 붙으므로 ②③④가 값에 하는 처리를 하나도 거치지 않는다. ⑤가 문장을 받았을 때는 이미 문법의 일부이고, 원래 값이었는지 구분할 방법이 없다.

**`${}`가 필요한 이유는 `?`를 아무 데나 쓸 수 없기 때문이다.** `?`는 값이 오는 자리에만 놓을 수 있다. 테이블명, 컬럼명, `ASC`/`DESC` 같은 키워드는 값 표현식이 아니라 문장의 구조이므로 바인딩 대상이 아니다. 이 자리를 동적으로 바꾸려면 SQL 문장 자체를 다르게 만드는 수밖에 없고, 그게 `${}`다.

**`${}`는 화이트리스트로만 쓴다.** 미리 정해 둔 허용 목록에 있는 값인지 확인하고, 없으면 기본값으로 떨어뜨린다. 반대로 위험한 문자와 패턴을 걸러내는 블랙리스트 방식은 우회 방법이 계속 나온다.

## 예시

`#{}`로 되는 자리와 안 되는 자리를 차례로 본다.
...
②의 결과다. 값 `홍길동`은 ③④를 거쳐 `?` 자리로 넘어간다. 따옴표는 붙이지 않는다.
...
①에서 끝난 결과다. 문자열이므로 따옴표를 직접 써야 한다. 그리고 이 따옴표가 뚫린다. `name`으로 `' OR '1'='1`이 들어오면 이렇게 된다.
...
```java
List<User> findAll(@Param("orderBy") String orderBy, @Param("direction") String direction);
```
...
①에서 문자열이 그대로 박히므로, 걸러 내는 일은 이 자바 코드가 전부다. `input`과 `dir`은 요청에서 받은 원본 값이다.

### LIKE 패턴 — `#{}`

와일드카드는 값의 일부이므로 `#{}`로 넘긴다. 매퍼 안에서 붙이려면 [`<bind>`](https://mybatis.org/mybatis-3/dynamic-sql.html#bind)를 쓴다. `value` 안의 이름은 파라미터 이름으로 해석되므로 `@Param`으로 이름을 붙여 둔다.
...
②의 결과는 `... WHERE name LIKE ?` 하나다. `%`는 값에 붙어 ③④로 넘어간다.

### IN 목록 — `<foreach>` + `#{}`

[`<foreach>`](https://mybatis.org/mybatis-3/dynamic-sql.html#foreach)로 원소 개수만큼 `?`를 만든다. `collection`에 쓸 이름은 파라미터가 그 리스트 하나뿐이고 `@Param`이 없을 때만 `list`가 된다. `@Param`을 붙이면 `list`는 통하지 않고 붙인 이름만 쓸 수 있다.
...
원소가 셋이면 ②가 `IN (?, ?, ?)`를 만든다. `?` 하나에는 값 하나만 바인딩되므로 `IN (#{ids})`로는 리스트를 넘길 수 없다.

## 혼동하기 쉬운 것

**`#{}`만 쓰면 실행계획이 자동으로 재사용된다는 오해.** `#{}`가 하는 일은 ②에서 `?`를 만드는 데까지다. 그 `?`를 ④에서 어떻게 다룰지, 그래서 ⑤가 문장을 재사용할지는 드라이버와 그 설정이 정한다.

**`#{}`에 따옴표를 붙이면 안 된다.** MyBatis는 따옴표 안팎을 가리지 않고 `#{name}`을 `?`로 바꾸므로 `'#{name}'`은 `'?'`가 된다. 문자열 리터럴 안의 `?`는 ④에서 드라이버가 파라미터 자리로 보지 않는다. 넘길 값은 있는데 받을 자리가 없어 실패한다.

**`ORDER BY #{orderBy}`는 컬럼을 가리키지 못한다.** `ORDER BY ?`가 만들어지고 컬럼명이 아니라 문자열 값이 바인딩된다. ⑤에서 에러를 내는지는 DBMS마다 다른데, 넘어가더라도 모든 행이 같은 값을 정렬 기준으로 갖는 셈이라 의도한 순서가 나오지 않는다. 어느 쪽이든 `${}`로 바꿔야 한다.

## 구현체별 차이

②가 만든 `?`를 ④에서 드라이버가 어떻게 처리하는지는 제품과 설정에 따라 다르다.
...
- [PreparedStatement의 ?는 어디서 값과 만나는가](/posts/jdbc-driver-value-paths/) — ④에서 드라이버가 값을 다루는 두 경로를, 서버가 실제로 받은 문장을 잡아 확인한 기록
