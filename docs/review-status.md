...
검수할 때 도입 섹션 제목을 같이 본다. 발행 글 대부분이 `## 왜 필요한가`를 쓰고 있는데,
실제 내용은 「무엇이 문제인가」나 「풀리지 않는 질문」인 경우가 섞여 있다. 선택지는 각 템플릿의 도입 섹션에 있다.
체크된 글은 이 정리가 끝난 글이다. 아직 체크되지 않은 글은 그 글을 검수할 때 함께 고친다.
(`postgresql-numeric-vs-double`은 본문 84번째 줄에서 「왜 필요한가」를 이름으로 참조한다. 제목을 바꾸면 그 줄도 같이 고친다.)
