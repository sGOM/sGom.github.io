...
값의 의미를 정하는 것은 처리기(processor)나 JSON을 쓰는 개별 명세의 몫이라는 뜻이다.
...
**중복된 이름.** 이름이 겹쳐도 문법 위반이 아니다. ECMA-404는 이름으로 쓰이는 문자열에 아무 제약도 두지 않는다고 명시한다.
...
RFC 8259는 객체 안의 이름이 유일해야 한다(SHOULD)고 한 단계 더 나가지만, 겹쳤을 때의 결과는 정하지 않고 실제 구현을 열거만 한다.
...
**숫자의 정밀도.** JSON 문법의 숫자는 자릿수 제한이 없다. 어떤 수 타입에 담을지는 파서가 정한다. RFC 8259 6절은 표준을 바꾸는 대신 권고를 남겼다.
...
[배정밀도 부동소수점](https://en.wikipedia.org/wiki/Double-precision_floating-point_format)(`double`)에 들어가는 범위를 벗어나지 말라는 말이지, 벗어난 텍스트가 무효라는 말이 아니다.

**문법 밖의 수.** RFC 8259 6절은 문법으로 표현할 수 없는 수치를 허용하지 않는다.

> Numeric values that cannot be represented in the grammar below (such as Infinity and NaN) are not permitted.

여기서 입력과 출력의 취급이 갈린다. 9절 Parsers는 파서가 문법에 맞는 텍스트를 전부 받아들여야 한다(MUST)고 하면서도, JSON이 아닌 형식이나 확장을 받아주는 것까지는 허용한다(MAY).
...
반면 10절 Generators는 이 두 문장이 전부다.

> A JSON generator produces JSON text. The resulting text MUST strictly conform to the JSON grammar.

같은 `NaN`이라도 읽을 때와 쓸 때의 판정이 갈린다. `NaN`을 받아들이는 파서는 9절의 MAY가 열어둔 확장이라 표준을 어기지 않는다. `NaN`을 내보내는 직렬화기는 10절의 MUST를 어긴다.

**JavaScript 문자열 리터럴과의 어긋남.** JSON 문자열 안에는 U+2028(LINE SEPARATOR)과 U+2029(PARAGRAPH SEPARATOR)를 이스케이프 없이 넣을 수 있다. ES2018까지의 JavaScript 문자열 리터럴에는 넣을 수 없었다. [tc39/proposal-json-superset](https://github.com/tc39/proposal-json-superset)은 ECMAScript가 JSON을 부분집합으로 주장하지만 사실이 아닌 이유를 이렇게 적는다.
...
이 제안이 ES2019에 들어가면서 이 차이는 사라졌다. 다만 상위집합이 성립하는 자리는 표현식과 문자열 리터럴이다. `{`로 시작하는 JSON 텍스트는 문(statement) 자리에서 객체가 아니라 블록으로 읽혀 지금도 문법 오류다.
...
| `1E400` | `inf` | `Infinity` |
| `NaN` 직렬화 | `NaN` (RFC 8259 10절 위반) | `null` |
...
`NaN`은 표에서 유일하게 표준 위반이 걸리는 행이다. Python이 기본값으로 내보내는 `NaN`은 RFC 8259 10절이 요구하는 문법에 맞지 않고, 그렇게 만든 텍스트를 Node.js는 거부한다. `json.dumps(float("nan"), allow_nan=False)`로 막으면 `ValueError: Out of range float values are not JSON compliant: nan`이 난다. 다른 쪽 방향인 `json.loads("NaN")`은 9절의 MAY가 허용하는 확장이다.
...
값이 갈리는 것은 파싱이 끝난 뒤에는 손볼 수 없다. 그 시점에는 `12345678901234567000`이 반올림된 값인지 원래 그 값이었는지 구분할 방법이 없다. 그래서 대응은 보내는 쪽이 표현을 바꾸거나, 받는 쪽이 파서를 바꾸는 두 방향뿐이다.

| 방법 | 고치는 자리 | 얻는 것 | 잃는 것 |
|---|---|---|---|
| 큰 수와 금액을 문자열로 싣기 | 보내는 쪽 | 파서 정밀도와 무관해진다 | 받는 쪽에서 변환 코드가 필요하다 |
| 파서 옵션으로 타입 지정 (`json.loads(text, parse_float=Decimal)`) | 받는 쪽 | 소수 정밀도를 지킨다 | 언어마다 옵션이 달라 받는 쪽마다 따로 설정해야 한다 |
| [JSON Schema](https://json-schema.org/)로 입력 검증 | 받는 쪽 입구 | 타입과 범위를 명시할 수 있다 | 중복 키처럼 파싱 단계에서 이미 결정된 것은 막지 못한다 |
| 이진 포맷(Protocol Buffers 등)으로 교체 | 양쪽 합의 | 타입이 스키마에 못박힌다 | 사람이 읽을 수 없고 스키마 배포가 필요하다 |
