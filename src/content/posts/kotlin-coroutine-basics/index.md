...
스레드는 하나 만드는 데 수 KB\~수 MB의 스택 메모리가 들고, 컨텍스트 스위칭 비용도 OS 스케줄러가 담당한다.

네트워크 요청처럼 대기 시간이 긴 작업을 스레드로 처리하면, 스레드는 대기 중에도 자원을 점유한 채 블로킹된다. 동시에 처리할 작업이 수천 개로 늘면 스레드 수도 그만큼 늘어나 메모리와 스케줄링 비용이 커진다.
...
**CoroutineScope**는 코루틴을 개별로 관리하지 않고 묶어서 관리하기 위한 경계다. 화면이 사라질 때 그 화면에서 시작한 코루틴을 전부 취소하려면 화면과 스코프의 생명주기를 맞춰둔다. 이렇게 스코프 단위로 취소가 전파되는 구조를 구조화된 동시성(structured concurrency)이라 부른다.
